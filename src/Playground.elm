module Playground exposing
    ...
    , Number
    , Animation, Game, Msg, changeMemory, getMemory, initGame, init_, subscriptionsGame, subscriptions_, updateGame, update_, viewGame, view_
    )
...
-}
view_ : List Shape -> Screen -> Browser.Document msg
view_ shapes screen =
    { title = "Playground"
...
init_ : () -> ( Screen, Cmd msg )
init_ () =
    ( toScreen 600 600, Cmd.none )


update_ : ( Int, Int ) -> a -> ( Screen, Cmd msg )
update_ ( width, height ) _ =
    ( toScreen (toFloat width) (toFloat height)
    , Cmd.none
    )


subscriptions_ : a -> Sub ( Int, Int )
subscriptions_ _ =
...
game viewMemory updateMemory initialMemory =
    Browser.document
        { init = initGame initialMemory
        , view = viewGame viewMemory
        , update = updateGame updateMemory
        , subscriptions = subscriptionsGame
        }


subscriptionsGame : Game memory -> Sub Msg
subscriptionsGame (Game visibility _ _) =
    case visibility of
        E.Hidden ->
            E.onVisibilityChange VisibilityChanged
...
updateGame : (Computer -> memory -> memory) -> Msg -> Game memory -> ( Game memory, Cmd msg )
updateGame updateMemory msg model =
    ( gameUpdate updateMemory msg model
    , Cmd.none
    )


initGame : memory -> ( Int, Int ) -> ( Game memory, Cmd msg )
initGame initialMemory flags =
    ( Game E.Visible initialMemory (initialComputer flags)
    , Cmd.none
      -- , Task.perform GotViewport Dom.getViewport
    )


viewGame : (Computer -> memory -> List Shape) -> Game memory -> Browser.Document msg
viewGame viewMemory (Game _ memory computer) =
    let
        newComputerScreen =
            { width = 800
            , height = 600
...
initialComputer : ( Int, Int ) -> Computer
initialComputer ( x, y ) =
...
changeMemory : Game memory -> (memory -> memory) -> Game memory
changeMemory (Game visibility memory computer) memoryChanger =
    Game visibility (memoryChanger memory) computer


getMemory : Game memory -> memory
getMemory (Game visibility memory computer) =
    memory
